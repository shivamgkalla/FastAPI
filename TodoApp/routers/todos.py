...
async def render_todo_page(request: Request, db: db_dependency):
    try:
        ...
        todos = db.query(Todos).filter(Todos.owner_id == user.get("id")).all() # type: ignore
        # 🔍 Debug: print each todo's completion status and type
        for todo in todos:
            print(f"{todo.title} - complete: {todo.complete} ({type(todo.complete)})")
...
async def render_edit_todo_page(request: Request, todo_id: int, db: db_dependency):
    try:
        ...
        todo = db.query(Todos).filter(Todos.id == todo_id).first() # type: ignore
